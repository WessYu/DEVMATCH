...
function fallbackAvatar(email: string) {
  ...
  const avatarId = fallbackAvatarIds[seed % fallbackAvatarIds.length];
  return `https://images.unsplash.com/${avatarId}?auto=format&fit=crop&w=720&h=720&q=70`;
}
...
function buildSignals(input: DeveloperProfileInput) {
  ...
  if (source.includes("acessibilidade")) signals.add("Acessibilidade");
  if (source.includes("performance")) signals.add("Performance");
...
export async function saveOwnedDeveloperProfile(email: string, input: DeveloperProfileInput) {
  ...
  const profile: DeveloperProfile = {
    ...
    avatar: input.avatar || fallbackAvatar(email),
    bio: input.bio,
    salary: input.salary,
